...
Web 版は上部中央のタブで左半分だけを切り替える（CLOCK / TIMER / POMODORO / ALARM）。
右半分のデジタル時計と残量バーはどのタブでも同じものが出たままなので、
タイマーやポモドーロを使いながら現在時刻と残り時間を見続けられる。

- **TIMER**: 電子レンジ式の直接入力。キーパッド（または物理キーボード）で
  `2000` と打てば 20:00、`13000` なら 1:30:00。Enter か START で開始、残量バーつきで、
  0 になると止めるまで画面全体の点滅・ビープ音・タブタイトルの点滅で
  知らせ続ける（STOP か ESC で停止）。アラームとポモドーロも同じ。
  ブラウザのタブやウィンドウが背面のときは、許可があれば OS の通知でも知らせる
  （通知の許可はアラーム登録時に求める）
  一時停止・リセット可。動作中のままリロードしても続きから動く
- **POMODORO**: 25分の集中 × 4ラウンド、合間に5分・4ラウンド目の後に15分の休憩。
  フェーズの切り替わりで鳴動して自動で次へ進む。ラウンドの進みはドットで表示、
  SKIP / RESET つき。現在時刻と残り時間は右半分に出たままなので重複して置かない
- **ALARM**: `+ NEW ALARM` で時刻を数字4桁（HHMM）で登録し、繰り返す曜日を選ぶ。
  曜日を選ばなければ 1 回だけ鳴り、鳴り終わると自動で OFF になる。
  一覧の行を押せば時刻と曜日を編集でき、ON/OFF 切替・削除も可。
  表記は 24H/12H 設定に追従する。
  ※ 音が鳴るのはページ（またはインストールした PWA）を開いているあいだだけ

以下は CLOCK 画面（とネイティブ版）の機能。
...
- 「今日の残り」「今月の残り」「今年の残り」「目標年齢までの残り」を
  ...
  Web 版は行のクリックで開く入力ダイアログから設定してブラウザにだけ保存し、
...
- **Android**: 公開 URL を Chrome で開いて **⋮ → ホーム画面に追加**
  （または「アプリをインストール」）。iPad と同じく全画面の独立アプリになる
- **フルスクリーン**: フッター右端の ⛶ アイコンで全画面表示に切り替わる
  （もう一度押すか ESC で解除）
