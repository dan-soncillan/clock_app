...
Web 版は上部のタブで 3 つの画面を切り替えられる（CLOCK / TIMER / ALARM）。

- **TIMER**: `+10S`〜`+1H` のボタンで時間を積んで START。残量バーつきで、
  0 になると画面の点滅とビープ音で知らせる（STOP か ESC で停止、2分で自動停止）。
  一時停止・リセット可。設定はブラウザに保存され、動作中のままリロードしても続きから動く
- **ALARM**: `+ NEW ALARM` で時刻を数字4桁（HHMM）で登録。有効にしているあいだ
  毎日その時刻に鳴る。ON/OFF 切替・削除可、一覧はブラウザに保存される。
