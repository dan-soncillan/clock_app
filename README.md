...
- 「今日の残り」「今月の残り」「今年の残り」「目標年齢までの残り」を
  数値と残量バーで表示（バーはいずれも右端から減っていく向き）

  一番下のバーの基準になる生年月日は**コードには含めない**。
  Web 版はラベルのクリックで入力してブラウザにだけ保存し、
  ネイティブ版は設定（⌘,）で入力して端末の UserDefaults にだけ保存する。
  未設定のあいだは値を「—」で伏せる。目標年齢（80）は
  `Views/RootView.swift` / `web/index.html` の定数で変えられる。
